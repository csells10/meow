# GameLens Learning Light Architecture
...
**Historical implementation source:** current `dev` and `documentation/live`

---

## 1. Decision

GameLens will pursue a smaller learning architecture named **Learning Light**.

Learning Light starts with what already works on `main`, selectively reuses proven safety boundaries from `dev`, and postpones operational infrastructure that has not earned its maintenance cost.
...
The current `dev` branch is therefore best understood as an operationalization prototype around existing intelligence. Its most important new product datum is the immutable pregame snapshot. Much of the remaining work is storage coordination, receipts, run visibility, QA, and documentation.
...
Learning Light should allow GameLens to answer, over time:
...
Learning Light does not initially require:
...
Learning Light must preserve the ability to build League Discovery later even if no League Discovery deliverable is scheduled.
...
Learning Light must not remove, flatten away, or stop persisting these fields.
...
Learning Light should make a future algorithm possible without pretending that an algorithm is already justified.
...
7. updates the Learning Light README and Sprint checkpoint.
...
Where an older packet plan conflicts with this document about what should be built next, Learning Light governs future scope. The older document remains the authority for what that packet proved at its historical checkpoint.
